...
describe('session title fallback', () => {
  it('skips harness-injected machinery when falling back to the first prompt (titleOf unknown)', () => {
    let state = applyEnvelope(startingState(), frame('s1', 'session.started', {}), NOW - 90_000);
    state = appendUserMessage(
      state,
      '<local-command-caveat>Caveat: the messages below were generated by the user…',
      NOW - 80_000,
    );
    state = appendUserMessage(state, 'fix the race', NOW - 70_000);
    state = applyEnvelope(
      state,
      frame('s1', 'agent.permission_request', {
        requestId: 'req-1',
    ...
    const asks = buildInboxAsks({
      live: liveWith(['s1', state]),
      titleOf: () => null,
      deviceNameOf: () => null,
    });
    expect(asks[0]!.sessionTitle).toBe('fix the race');
  });
});
